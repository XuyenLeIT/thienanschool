...
class AccountController extends Controller
{
    ...
    public function index()
    {
        ...
        $accounts = Account::query()
            ->where('id', '!=', $authUser->id) // không lấy chính mình
            ->when($authUser->role === 'manager', function ($query) {
                // nếu login là manager -> ẩn admin và manager khác
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            if ($request->hasFile('avatar'))
                $account->avatar = $this->handleAvatarUpload($request->file('avatar'));
    ...
    public function edit($id)
    {
        ...
        if ($authUser->role === 'manager')
            $roles = array_diff($roles, ['manager', 'admin']);
        $classGrades = Account::$classGrades;
        return view('admin.accounts.edit', compact('account', 'roles', 'classGrades'));
    }
    ...
    public function update(Request $request, $id)
    {
        try {
            ...
            if ($request->hasFile('avatar'))
                $account->avatar = $this->handleAvatarUpload($request->file('avatar'), $account->avatar);
            $account->save();
            return back()->with('success', 'Account tạo thành công và email đã gửi.');
    ...
    public function show($id)
    {
        $account = Account::findOrFail($id);
        $authUser = session('auth_user');
        return view('admin.accounts.show', compact('account', 'authUser'));
    }
